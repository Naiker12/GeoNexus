...
def _flash_attn_submodule_exists(name):
    ...
    `flash_attn/__init__.py` (and load `flash_attn_2_cuda`) during `import spartan_agent`, for users
...
def fix_transformers5_bare_annotation_configs():
    ...
    takes ``import spartan_agent`` down with it. Patching
...
def check_transformers_dependency_versions():
    ...
    try:
        ...
        # Nothing here is worth failing `import spartan_agent` over.
        if importlib.util.find_spec("transformers") is None:
            return
    except Exception:
        return
...
def _torchcodec_version_mismatch_hint() -> str | None:
    ...
    try:
        ...
        # Non-PEP440 version strings must never break `import spartan_agent`.
        return None
...
def fix_torchao_torch_symbol_skew():
    """Let `import spartan_agent` survive a torchao built for a newer torch.
...
def propagate_torchao_fix_to_subprocesses():
    ...
    `import spartan_agent` there would pay the full import cost each time and could
...
# and the alias resolves lazily so `import spartan_agent` pays nothing.
