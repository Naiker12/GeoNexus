...
import nexus_cli.commands.chat as chatmod
from nexus_cli._inference import (
    ChatBackend,
    HttpChatBackend,
    collect_stream,
...
def test_find_studio_server_none_when_not_running(monkeypatch):
    ...
    from nexus_cli import _inference
...
def test_padded_body_helper_passes_a_real_payload_through():
    from nexus_cli._inference import require_completed_padded_body
...
def test_deferred_error_helper_passes_a_normal_body_through():
    from nexus_cli._inference import raise_for_deferred_error
...
def _stub_studio_gguf_load(monkeypatch):
    ...
    import nexus_cli._inference as inference
...
def test_inference_forwards_gguf_runtime_options_to_loader(monkeypatch):
    from nexus_cli.commands import inference as infermod
...
def test_inference_local_handles_stream(monkeypatch, chunk_kind, expected_exit):
    from nexus_cli.commands import inference as infermod
    from nexus_cli._inference import ensure_studio_backend_path
