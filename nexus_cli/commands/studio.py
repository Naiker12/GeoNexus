...
from nexus_cli import _studio_deps, _studio_runtime_gate
from nexus_cli._inference import SpeculativeType
from nexus_cli.commands import _password_prompt
...
_CMD_SHIM_MARKERS = (b"unsloth-studio-managed-launcher", b"from nexus_cli import app")
...
#
# `from nexus_cli import app` rather than a find_spec, deliberately. find_spec
...
_MANAGED_CLI_IMPORT_PROBE = (
    "import sys, os; sys.path[:1] = [x for x in sys.path[:1] if getattr(sys.flags, 'safe_path', False) or x not in ('', os.getcwd())]; "
    "from nexus_cli import app; sys.exit(0)"
)
...
def _emit_run_cloudflare_notice(
    ...
    from nexus_cli._tool_policy import is_external_host
...
def _managed_cli_package_present(python: Path) -> bool:
    ...
    ``from nexus_cli import app``, so an orphaned ``unsloth-*.dist-info`` (a
    ...
    The probe runs the trampoline's own ``from nexus_cli import app`` rather
...
def _load_model_via_http(
    ...
    from nexus_cli._inference import raise_for_deferred_error, require_completed_padded_body
