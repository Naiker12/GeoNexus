...
from spartan_agent_cli import _studio_deps, _studio_runtime_gate
from spartan_agent_cli._inference import SpeculativeType
from spartan_agent_cli.commands import _password_prompt
...
_CMD_SHIM_MARKERS = (b"unsloth-studio-managed-launcher", b"from spartan_agent_cli import app")
...
_MANAGED_CLI_IMPORT_PROBE = (
    "import sys, os; sys.path[:1] = [x for x in sys.path[:1] if getattr(sys.flags, 'safe_path', False) or x not in ('', os.getcwd())]; "
    "from spartan_agent_cli import app; sys.exit(0)"
)
...
def _emit_run_cloudflare_notice(
    ...
    from spartan_agent_cli._tool_policy import is_external_host
...
def _load_model_via_http(
    ...
    from spartan_agent_cli._inference import raise_for_deferred_error, require_completed_padded_body
