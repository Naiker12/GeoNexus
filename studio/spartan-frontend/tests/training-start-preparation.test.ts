...
// `_monitor_tqdm` (studio/spartan_backend/core/training/worker.py).
...
test("every status the worker sends reaches a row", () => {
  // Swept from the `_send_status`/`status_message` literals in studio/spartan_backend/core/training.
...
test("every tqdm description the dataset work emits reaches the dataset row", () => {
  // Swept from the `desc =` literals under studio/spartan_backend. `_monitor_tqdm` forwards these
