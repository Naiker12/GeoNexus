...
export function preflightStaleMessage(
  ...
  if (disposition === "owned_stale") {
    return "Desktop-owned Spartan backend is too old for this desktop app. Run `spartan studio update`, then restart Spartan.";
  }
  return "Managed Spartan install is too old. Run `spartan studio update`.";
}
