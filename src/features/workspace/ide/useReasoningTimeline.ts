...
import type { TimelineItem, TimelineStep } from "./timeline-types"
...
export function useReasoningTimeline(sessionId: string, initialSteps: TimelineStep[] = []) {
  ...
  const isRunning = React.useMemo(() => {
    ...
    return (
      events.some((e) => e.event_type === "pipeline_started") &&
      !events.some(
        (e) => e.event_type === "pipeline_completed" || e.event_type === "pipeline_failed"
      )
    )
  }, [events, sessionId])
