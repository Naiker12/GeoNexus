    ...
    <>
      <div className="flex items-end gap-2">
        <div className="flex-1">
          <FormInput
            ...
            placeholder={
              discoveredTools.length === 0 ? "buffer, distance, load_layer" : "Selecciona abajo"
            }
            value={toolsRaw}
      ...
      {selectedToolNames.size > 0 && (
        <p className="text-[10px] text-emerald-500 font-medium">
          {selectedToolNames.size} tool{selectedToolNames.size !== 1 ? "s" : ""} seleccionada
          {selectedToolNames.size !== 1 ? "s" : ""}
        </p>
      )}
    </>
  )
