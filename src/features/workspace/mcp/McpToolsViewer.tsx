...
function ToolStatusPill({ status }: { status: McpTool["status"] }) {
  return (
    ...
    >
      {status === "ready" && (
        <>
          <CheckCircle2Icon className="size-2.5" /> listo
        </>
      )}
      {status === "guarded" && (
        <>
          <ShieldAlertIcon className="size-2.5" /> protegido
        </>
      )}
      {status === "planned" && (
        <>
          <CalendarIcon className="size-2.5" /> planeado
        </>
      )}
    </span>
  )
