...
export function McpToolsViewer({ serverId }: McpToolsViewerProps) {
  if (!serverId) return null

  const server = mcpServers.find((s) => s.id === serverId)
  if (!server) return null

  const tools = mcpTools.filter((t) => t.server === serverId)
      ...
      <div className="flex items-center justify-between border-b border-border/60 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-semibold uppercase tracking-wider text-emerald-500">
            {server.name} — herramientas expuestas
          </h3>
        </div>
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <span className="mt-1 inline-block text-[9px] bg-muted text-muted-foreground font-semibold px-1.5 py-0.5 rounded uppercase tracking-wider scale-95 origin-left">
                  {tool.category}
                </span>
              </div>
            ...
            <div className="mt-3.5 rounded bg-[#090a0c] border border-border/40 p-2.5 font-mono text-[10px] leading-relaxed shadow-sm">
              ...
              <div className="py-1">
                ...
                <span className="text-[#e06c75] font-semibold">({tool.args || "void"})</span>
              </div>
              <div className="mt-1 pt-1.5 border-t border-white/[0.02] text-muted-foreground/80">
                ...
                <span className="text-[#98c379] font-medium">{tool.result}</span>
              </div>
            </div>
          </article>
        ))}
...
function ToolStatusPill({ status }: { status: "ready" | "guarded" | "planned" }) {
  return (
    <span
      className={cn(
