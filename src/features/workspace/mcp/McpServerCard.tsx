...
function errorHint(error?: string): ErrorHintData | null {
  ...
  if (error.includes("Conexión fallida"))
    return {
      icon: "\u26A0",
      label: "Servidor no iniciado",
      action: "instructions",
      actionLabel: "Ver instrucciones",
    }
  if (error.includes("Auth requerida") || error.includes("401"))
    return {
      icon: "\u{1F511}",
      label: "Token requerido",
      action: "configure",
      actionLabel: "Agregar token",
    }
  if (error.includes("Acceso denegado") || error.includes("403"))
    return {
      icon: "\u{1F512}",
      label: "Token sin permisos",
      action: "configure",
      actionLabel: "Cambiar token",
    }
  if (error.includes("no encontrado") || error.includes("404"))
    return { icon: "\u2753", label: "URL incorrecta", action: "edit", actionLabel: "Verificar URL" }
  if (error.includes("415"))
    return {
      icon: "\u2699",
      label: "Protocolo no compatible",
      action: "edit",
      actionLabel: "Verificar config",
    }
  if (error.includes("Timeout"))
    return {
      icon: "\u23F1",
      label: "Timeout — servidor lento",
      action: "ping",
  ...
  if (error.includes("Handshake OK"))
    return {
      icon: "\u26A0",
      label: "Online (sin tools/list)",
      action: "discover",
      actionLabel: "Descubrir tools",
    }
  if (error.includes("stdio"))
    return {
      icon: "\u26A1",
      label: "Servidor local (STDIO)",
      action: "discover",
      actionLabel: "Descubrir tools",
    }
  return null
...
export function McpServerCard({
  ...
  const hint =
    server.status === "offline"
      ? errorHint(server.last_error ?? server.last_ping_at ?? undefined)
      : null
  ...
  return (
    ...
    >
      {showDeleteConfirm && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-lg bg-white/95 dark:bg-zinc-950/95 px-4">
          <p className="text-sm text-center">
            ...
            <br />
            <span className="text-xs text-muted-foreground">Se borrarán sus tools y reglas.</span>
          </p>
      ...
      <div className="flex items-start justify-between gap-2.5">
        ...
        <span
          className={cn(
            "inline-flex h-6 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-[0.7rem] font-medium transition-colors border",
            cfg.badge
          )}
        >
      ...
      <div className="mt-2 grid grid-cols-3 gap-2">
        ...
        <MetricCol
          label="LATENCIA"
          value={server.latency_ms ? `${server.latency_ms}ms` : isStdio ? "N/A" : "\u2014"}
