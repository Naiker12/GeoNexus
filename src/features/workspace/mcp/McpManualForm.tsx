  ...
  return (
    <section className="grid gap-2.5 rounded-lg border border-border/80 bg-card/40 p-3 shadow-inner">
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          {form.transport === "http" ? (
            <FormInput
              label="URL"
              placeholder="localhost:3001 o https://..."
              value={form.url ?? ""}
      ...
      {form.transport === "stdio" && (
        <FormInput
          label="Args (separados por espacio)"
          placeholder="-y @modelcontextprotocol/server-memory"
          value={form.args?.join(" ") ?? ""}
          onChange={(v) => updateForm("args", v ? tokenizeArgs(v) : undefined)}
        />
      )}
      <FormInput
        ...
        value={form.auth_ref ?? ""}
        onChange={(v) => updateForm("auth_ref", v)}
      />
      <FormInputPassword
        label="Token de autenticación (opcional)"
        placeholder="sbp_xxxx o api_key"
        value={form.auth_token ?? ""}
        onChange={(v) => updateForm("auth_token", v)}
      />
      {(form.url ?? "").toLowerCase().includes("supabase.com/mcp") && (
        <div className="rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/50 px-3 py-2.5 text-xs">
          ...
          <ol className="list-decimal list-inside text-green-700 dark:text-green-400 space-y-0.5">
            ...
            <li>
              Genera un <strong>Personal Access Token</strong>
            </li>
            <li>
              Pégalo en <strong>Token de autenticación</strong> arriba
            </li>
          </ol>
          <p className="mt-1.5 text-green-600 dark:text-green-500">
            URL correcta:{" "}
            <code className="text-[10px] bg-green-100 dark:bg-green-900/50 px-1 rounded">
      ...
      <details className="rounded-md border border-border/60 bg-muted/30 px-2.5 py-1.5">
        ...
        <div className="mt-1.5 space-y-1.5 text-[10px] text-muted-foreground">
          <p>
            <strong>Supabase</strong>: genera PAT en{" "}
            <code className="text-primary">supabase.com/dashboard/account/tokens</code> y pégalo en
