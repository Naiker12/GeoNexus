...
export function AnalysisMetrics() {
    ...
    return (
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="animate-pulse rounded-lg border border-border/80 bg-card/95 p-3 shadow-sm backdrop-blur"
          >
  ...
  const items = error
    ...
    : [
        {
          label: "Tokens hoy",
          value: data ? formatTokens(data.tokens_hoy) : "0",
          detail: "Tokens totales del día",
        },
        {
          label: "Consultas IA",
          value: data ? String(data.consultas_ia) : "0",
          detail: "Respuestas del asistente hoy",
        },
        {
          label: "Costo estimado",
          value: data ? formatCost(data.costo_estimado) : "$0.00",
          detail: "Costo acumulado del día",
        },
        {
          label: "Trazas guardadas",
          value: data ? String(data.trazas_guardadas) : "0",
          detail: "Tool-calls registrados hoy",
        },
      ]
