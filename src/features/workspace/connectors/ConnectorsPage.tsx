...
} from "@/features/workspace/connectors/connector-types"
...
export function ConnectorsPage() {
  ...
  React.useEffect(() => {
    listConnectorConfigs("project-default")
      .then(setConfigs)
      .catch(() => {})
  }, [])
                ...
                <div className="flex min-w-0 items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 max-w-3xl text-xs leading-4 text-muted-foreground">
                      Conecta OneDrive, SharePoint y otras fuentes para cargar capas GIS, documentos
                      POT y resultados sin salir de Geo Agents.
                    </p>
                  </div>
                </div>
      ...
      <div className="mx-auto flex w-full max-w-[110rem] flex-col gap-3">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {providers.map((provider) => (
            <ConnectorCard key={provider.id} provider={provider} onSelect={openProvider} />
          ))}
        </section>
      </div>
