...
const summaries = [
  {
    label: "ONLINE",
    getValue: (connectors: AiConnector[]) => connectors.filter((c) => c.status === "online").length,
  },
  {
    label: "LOCAL",
  ...
  {
    label: "MCP",
    getValue: (connectors: AiConnector[]) => connectors.filter((c) => c.provider === "mcp").length,
  },
]
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-3 sm:grid-cols-4">
          {summaries.map((item) => (
            <div key={item.label}>
              ...
              <p className="mt-0.5 text-lg font-bold leading-5">{item.getValue(connectors)}</p>
            </div>
          ))}
        </div>
      </div>
