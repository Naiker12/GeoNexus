        ...
        <DialogHeader className="mb-0 border-b border-border px-4 pb-3 pt-4">
          <div className="flex items-start gap-2.5 pr-8">
            <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
              <ProviderBrandIcon providerId={option.id} fallback={option.icon} className="size-4" />
            </div>
            ...
            {models.length > 0 && (
              <>
                <p className="text-xs text-muted-foreground">
                  ...
                  {models.length !== 1 ? "s" : ""}. Selecciona el que quieras usar:
                </p>
                <ModelCombobox
...
function ModelCombobox({
  ...
  const filtered = React.useMemo(
    () =>
      query ? models.filter((m) => m.name.toLowerCase().includes(query.toLowerCase())) : models,
    [models, query]
  )
