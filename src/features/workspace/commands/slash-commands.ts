...
export const SLASH_COMMANDS: SlashCommandEntry[] = [
  ...
  {
    command: "/limpiar",
    description: "Limpia el historial de la conversación actual",
    params: "",
    example: "/limpiar",
    category: "General",
  },
  {
    command: "/grafica",
    description: "Genera una gráfica con los datos proporcionados",
    params: "[tipo] [datos]",
    example: "/grafica barras JS 65 Python 58",
    category: "Gráficas",
  },
  {
    command: "/chart",
    description: "Alias de /grafica, genera visualización de datos",
    params: "[tipo] [datos]",
    example: "/chart pastel JS 65 Python 58",
    category: "Gráficas",
  },
  {
    command: "/mapa",
    description: "Abre o consulta información en el mapa interactivo",
    params: "[consulta]",
    example: "/mapa zona Z1",
    category: "GIS",
  },
  {
    command: "/capa",
    description: "Muestra información sobre una capa GIS específica",
    params: "[nombre]",
    example: "/capa estratificación",
    category: "GIS",
  },
  {
    command: "/zona",
    description: "Consulta información de una zona territorial",
    params: "[nombre]",
    example: "/zona Zona Residencial Z1",
    category: "GIS",
  },
  {
    command: "/norma",
    description: "Busca normas o artículos del plan de ordenamiento",
    params: "[búsqueda]",
    example: "/norma alturas máximas",
    category: "Normas",
  },
  {
    command: "/documento",
    description: "Muestra contenido de un documento indexado",
    params: "[id]",
    example: "/documento doc-123",
    category: "Documentos",
  },
  {
    command: "/grafo",
    description: "Abre la red de conocimiento o consulta un nodo",
    params: "[nodo_id]",
    example: "/grafo node-zona-1",
    category: "Conocimiento",
  },
  {
    command: "/nodo",
    description: "Muestra el detalle de un nodo del grafo",
    params: "[id]",
    example: "/nodo node-norma-1",
    category: "Conocimiento",
  },
  {
    command: "/analizar",
    description: "Analiza un documento con IA",
    params: "[id]",
    example: "/analizar doc-456",
    category: "Análisis",
  },
  {
    command: "/resumir",
    description: "Genera un resumen del documento o conversación",
    params: "[id]",
    example: "/resumir doc-456",
    category: "Análisis",
  },
  {
    command: "/buscar",
    description: "Busca en todos los documentos indexados",
    params: "[términos]",
    example: "/buscar uso del suelo",
    category: "Búsqueda",
  },
  {
    command: "/conectar",
    description: "Conecta fuente de datos externa (WMS, shapefile)",
    params: "[tipo] [url]",
    example: "/conectar wms https://ejemplo.com/wms",
    category: "Datos",
  },
  {
    command: "/exportar",
    description: "Exporta resultado en JSON, CSV o PDF",
    params: "[formato]",
    example: "/exportar csv",
    category: "Datos",
  },
  {
    command: "/gis",
    description: "Abre el panel de herramientas GIS",
    params: "",
    example: "/gis",
    category: "GIS",
  },
  {
    command: "/ia",
    description: "Abre configuración de modelos de IA",
    params: "[modelo]",
    example: "/ia gpt-4o",
    category: "IA",
  },
...
export function handleSlashCommand(text: string): SlashResult {
...
export async function handleAsyncSlashCommand(text: string): Promise<SlashResult> {
  const trimmed = text.trim()
  if (!trimmed.startsWith("/")) return { handled: false }
    ...
    case "documento": {
      if (!args) {
        ...
        if (docs.length === 0) {
          return {
            ...
            type: "message",
            content:
              "No hay documentos indexados. Usa la sección **Documentos** para subir e indexar archivos.",
          }
        }
        const rows = docs.map(
          (d) =>
      ...
      if (!doc) {
        return {
          handled: true,
          type: "message",
          content: `No se encontró el documento \`${args}\`. Usa \`/documento\` para listar los disponibles.`,
        }
      }
      const chunks = await listDocumentChunks(args)
      const previewLines = chunks
        .slice(0, 3)
        .map(
          (c) =>
    ...
    case "analizar": {
      if (!args) {
        ...
        const docs = assets.filter(
          (a) =>
            ["document", "word", "excel", "pdf", "txt"].includes(a.kind) || a.status !== "ready"
        )
        if (docs.length === 0) {
          return {
            handled: true,
            type: "message",
            content:
              "No hay documentos disponibles para analizar. Sube documentos desde la sección **Documentos**.",
          }
        }
        const rows = docs.map((d) => `  • \`${d.id}\` — **${d.name}** (${statusLabel(d.status)})`)
        return {
          handled: true,
          type: "message",
      ...
      if (doc.status === "indexing") {
        return {
          handled: true,
          type: "message",
          content: `El documento **${doc.name}** ya está siendo indexado. Espera a que termine.`,
        }
      }
      if (doc.status === "ready") {
        const chunks = await listDocumentChunks(args)
    ...
    case "resumir": {
      if (!args) {
        return {
          ...
          type: "message",
          content:
            "Usa `/resumir [id]` con el ID del documento que quieres resumir. Ejemplo: `/resumir doc-123`\n\nUsa `/documento` para listar los documentos disponibles.",
        }
      }
      const doc = await getDataAsset(args)
      if (!doc) {
        return {
          handled: true,
          type: "message",
          content: `No se encontró el documento \`${args}\`.`,
        }
      }
      if (doc.status !== "ready") {
        return {
          handled: true,
          type: "message",
          content: `El documento **${doc.name}** no está indexado aún. Usa \`/analizar ${args}\` para indexarlo primero.`,
        }
      }
      const chunks = await listDocumentChunks(args)
      if (chunks.length === 0) {
        return {
          handled: true,
          type: "message",
          content: `El documento **${doc.name}** no tiene fragmentos. Reindexa con \`/analizar ${args}\`.`,
        }
      }
    ...
    case "buscar": {
      if (!args) {
        return {
          ...
          type: "message",
          content:
            "Usa `/buscar [términos]` para buscar en todos los documentos indexados. Ejemplo: `/buscar uso del suelo`",
        }
      }
      const assets = await listDataAssets()
      ...
      if (matches.length === 0) {
        return {
          handled: true,
          type: "message",
          content: `No se encontraron documentos que coincidan con "${args}".`,
        }
      }
      const rows = matches.map((d) => `  • \`${d.id}\` — **${d.name}** (${d.chunks} fragmentos)`)
      return {
        handled: true,
        type: "message",
    ...
    case "norma": {
      if (!args) {
        return {
          ...
          type: "message",
          content:
            "Usa `/norma [búsqueda]` para buscar normas del plan de ordenamiento. Ejemplo: `/norma alturas máximas`",
        }
      }
      ...
      if (!result.nodes || result.nodes.length === 0) {
        return {
          handled: true,
          type: "message",
          content: `No se encontraron normas que coincidan con "${args}".`,
        }
      }
      const rows = result.nodes.map(
        (n: GraphNode) =>
    ...
    case "grafo": {
      if (args) {
        ...
        if (!result.nodes || result.nodes.length === 0) {
          return {
            handled: true,
            type: "message",
            content: `No se encontraron nodos que coincidan con "${args}".`,
          }
        }
        const rows = result.nodes.map(
          (n: GraphNode) =>
      ...
      if (nodes.length === 0) {
        return {
          handled: true,
          type: "message",
          content: "El grafo de conocimiento está vacío. Indexa documentos para poblarlo.",
        }
      }
      ...
      const topRows = top.map(
        (n) => `  • \`${n.id}\` — **${n.label}** (usado ${n.use_count} veces)`
      )
      return {
        handled: true,
        type: "message",
    ...
    case "nodo": {
      if (!args) {
        return {
          ...
          type: "message",
          content:
            "Usa `/nodo [id]` con el ID del nodo que quieres consultar. Ejemplo: `/nodo node-norma-1`\n\nUsa `/grafo` para listar nodos disponibles.",
        }
      }
      const result = await searchGraphNodes("project-default", args, undefined, 5)
      const node = result.nodes?.find(
        (n: GraphNode) => n.id === args || n.label.toLowerCase().includes(args.toLowerCase())
      )
      if (!node) {
        return {
          handled: true,
          type: "message",
          content: `No se encontró el nodo \`${args}\`. Usa \`/grafo\` para explorar.`,
        }
      }
      return {
        handled: true,
        ...
        content: [
          ...
          node.evidence
            ? `**Evidencia:** ${node.evidence.slice(0, 300)}${node.evidence.length > 300 ? "…" : ""}`
            : "",
        ].join("\n"),
      }
    }
    ...
    case "exportar": {
      ...
      if (!["json", "csv", "pdf", "md"].includes(format)) {
        return {
          handled: true,
          type: "message",
          content: `Formato no soportado: "${format}". Usa: json, csv, pdf o md.`,
        }
      }
      return {
        handled: true,
    ...
    case "conectar": {
      ...
      if (!["wms", "shapefile", "folder"].includes(type.toLowerCase())) {
        return {
          handled: true,
          type: "message",
          content: `Tipo "${type}" no soportado. Usa: wms, shapefile o folder.`,
        }
      }
      return {
        handled: true,
