...
export function AgentLifeIndicator({
  status,
  skillName,
  conversationCount,
}: AgentLifeIndicatorProps) {
  return (
    <div className="flex items-center gap-2 px-1">
      <div className="flex size-5 items-center justify-center">
        <GeoAgentsIcon className="size-4" variant="nexus" />
      </div>
      <span className="text-[13px] font-semibold text-foreground ml-0.5 truncate">GeoAgents</span>
      ...
      >
        {status === "using_skill" && skillName ? `Aplicando ${skillName}...` : STATUS_TEXT[status]}
      </span>
      {status === "thinking" && (
        <span className="flex gap-0.5">
          <span
            className="size-1 animate-bounce rounded-full bg-emerald-500/60"
