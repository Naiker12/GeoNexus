import type { ListLlmModelsInput, LlmModelInfo } from "../types/llm"
...
export async function listLlmModels(input: ListLlmModelsInput): Promise<LlmModelInfo[]> {
...
export async function pingLlmProvider(config: LlmProviderConfig): Promise<LlmPingResult> {
  const invoke = await getInvoke()
  if (!invoke)
    return { status: "error", provider_type: config.provider_type, message: "Tauri not available" }
  return invoke("ping_llm_provider", { config })
}

// ── sendLlmMessage ────────────────────────────────────────────────────────────

export async function sendLlmMessage(request: LlmChatRequest): Promise<LlmChatResult> {
  const invoke = await getInvoke()
  if (!invoke)
    return { status: "error", provider_type: request.provider_type, message: "Tauri not available" }
