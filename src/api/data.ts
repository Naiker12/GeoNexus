...
export function listDataAssets(
  ...
  if (!projectId.trim()) throw new Error("projectId requerido")
  return invokeOrFallback("list_data_assets", { projectId }, [])
}

export function getDataAsset(assetId: string): Promise<DataAsset | null> {
  if (!assetId.trim()) throw new Error("assetId requerido")
  return invokeOrFallback("get_data_asset", { assetId }, null)
}
...
export function deleteDataAsset(assetId: string): Promise<void> {
  if (!assetId.trim()) throw new Error("assetId requerido")
  return invokeRequired<void>("delete_data_asset", { assetId })
}
...
export function validateDataAsset(assetId: string): Promise<AssetValidation> {
  ...
  return invokeOrFallback("validate_data_asset", { assetId }, fallback)
}

export function indexDocument(documentId: string): Promise<number> {
  if (!documentId.trim()) throw new Error("documentId requerido")
  return invokeRequired<number>("index_document", { documentId })
}

export function listDocumentChunks(documentId: string): Promise<DocumentChunk[]> {
  if (!documentId.trim()) throw new Error("documentId requerido")
  return invokeOrFallback<DocumentChunk[]>("list_document_chunks", { documentId }, [])
}

export async function listGraphNodes(projectId = DEFAULT_PROJECT_ID): Promise<GraphNode[]> {
  const nodes = await invokeOrFallback<BackendGraphNode[] | null>("list_graph_nodes", { projectId }, null)
  if (nodes) {
...
export async function listGraphEdges(projectId = DEFAULT_PROJECT_ID): Promise<GraphEdge[]> {
  const edges = await invokeOrFallback<BackendGraphEdge[] | null>("list_graph_edges", { projectId }, null)
  if (edges) {
...
export async function rebuildKnowledgeGraph(projectId = DEFAULT_PROJECT_ID): Promise<void> {
  await invokeOrFallback("rebuild_knowledge_graph", { projectId }, undefined)
}
