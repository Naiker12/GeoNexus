...
def test_preset_interface_includes_load_config():
    policy = _read("studio/spartan-frontend/src/features/chat/presets/preset-policy.ts")
    assert "loadConfig?: PresetLoadConfig" in policy


def test_preset_save_captures_load_config():
    sheet = _read("studio/spartan-frontend/src/features/chat/chat-settings-sheet.tsx")
...
def test_persisted_preset_serializes_load_config():
    storage = _read("studio/spartan-frontend/src/features/chat/utils/chat-settings-storage.ts")
    assert "normalizePresetLoadConfig(item.loadConfig)" in storage
    api = _read("studio/spartan-frontend/src/features/chat/api/chat-settings-api.ts")
    assert "loadConfig?: Record<string, unknown>" in api


def test_capture_reads_gguf_loaded_context():
    source = _read("studio/spartan-frontend/src/features/chat/presets/preset-load-config.ts")
...
def test_hydration_does_not_replay_preset_load_config():
    store = _read("studio/spartan-frontend/src/features/chat/stores/chat-runtime-store.ts")
...
def test_backend_chat_preset_accepts_load_config():
    routes = _read("studio/spartan_backend/routes/chat_history.py")
